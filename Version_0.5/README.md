...
This repository holds both the code (the code that is allowed to be released due to limitations on completeness and publication rules) as well as the images produced by the code. For Version 0.5, you cannot make states in the shape of the actual U.S. state. For that capability, use Version 1.0+. 
